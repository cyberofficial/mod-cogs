...
class AlcherRPG:
    ...
    async def start(self, ctx):
        ...
        if not userinfo["class"] == "None" and not userinfo["race"] == "None":
            ...
            if answer1 == ">start":
                ...
                userinfo["in_party"] = []
                fileIO("data/rpgmodded/players/{}/info.json".format(user.id), "save", userinfo)
        ...
        await self.bot.say("Welcome to TSTD's RPG!\n\nMay i ask what race you are?\n`Choose one`\nOrc\nHuman\nTenti")
    ...
    async def _create_user(self, user, server):
        if not os.path.exists("data/rpgmodded/players/{}".format(user.id)):
            os.makedirs("data/rpgmodded/players/{}".format(user.id))
            new_account = {
                "name": user.name,
                "race": "None",
                "class": "None",
            ...
            fileIO("data/rpgmodded/players/{}/info.json".format(user.id), "save", new_account)
        userinfo = fileIO("data/rpgmodded/players/{}/info.json".format(user.id), "load")

def check_folders():
    if not os.path.exists("data/rpgmodded"):
        print("Creating data/rpgmodded folder...")
        os.makedirs("data/rpgmodded")

    if not os.path.exists("data/rpgmodded/players"):
        print("Creating data/rpgmodded/players folder...")
        os.makedirs("data/rpgmodded/players")
        transfer_info()

def transfer_info():
    players = fileIO("data/rpgmodded/players.json", "load")
    for user_id in players:
        os.makedirs("data/rpgmodded/players/{}".format(user_id))
        # create info.json
        f = "data/rpgmodded/players/{}/info.json".format(user_id)
