...
class oboobs:
    ...
    async def tiity(self, ctx):
        ...
        try:
            ...
            async with aiohttp.get(search) as r:
                result = await r.json()
                titty = random.choice(result)
                titty = "http://media.oboobs.ru/{}".format(titty["preview"])
        except Exception as e:
            await self.bot.reply("Error getting results.")
            return
        if not dis_nsfw:
            await self.bot.say("{}".format(titty))
        else:
    ...
    async def booty(self, ctx):
        ...
        try:
            ...
            async with aiohttp.get(search) as r:
                result = await r.json()
                booty = random.choice(result)
                booty = "http://media.obutts.ru/{}".format(booty["preview"])
        except Exception as e:
            await self.bot.reply("Error getting results.")
            return
        if not dis_nsfw:
            await self.bot.say("{}".format(booty))
        else:
            await self.bot.send_message(ctx.message.author, "{}".format(booty))
            if self.settings["nsfw_msg"]:
                await self.bot.reply("nsfw content is not allowed in this channel, instead I have send you a DM.")
    ...
    async def boob_knowlegde():
        ...
        # Upadate ass len
        print("Updating amount of booty...")
        ...
        while not done:
            ...
            await asyncio.sleep(2.5)
        print("Total amount of booty:", settings["ama_ass"])
...
def check_files():
    if not os.path.isfile(SETTINGS):
        print("Creating default boobs booty settings.json...")
